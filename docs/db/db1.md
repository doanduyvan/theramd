...
database_type: 'MySQL'
Note: 'CSDL web bán hàng mỹ phẩm: tách khách hàng và quản trị viên, hỗ trợ sản phẩm có biến thể, phân quyền admin, đơn hàng, thanh toán, tồn kho.'
}
...
Table admin_direct_permissions {
admin_id bigint [not null, ref: > admins.id]
permission_id bigint [not null, ref: > permissions.id]
...
product_type product_type [not null, default: 'variant']
...
Indexes {
variant_id [name: 'idx_variant_images_variant']
}
}
...
Indexes {
inci_name [name: 'idx_ingredients_inci']
}
}
...
Indexes {
product_id [name: 'idx_product_usage_steps_product']
(product_id, step_no) [unique, name: 'uk_product_step']
}
}
...
fulfillment_status [name: 'idx_orders_fulfillment_status']
...
condition_type coupon_condition_type [not null]
...
Indexes {
slug [name: 'idx_pages_slug']
status [name: 'idx_pages_status']
}
}
